...
if __name__ == '__main__':
    ...
    cfg = misc.load_config(args.config, 'configs/generator_testbed/default2.yaml')
    ...
    eval_z  = dataset[0][0].to(device)
    print(dataset)
    print(eval_z) # eval_z is becoming 0 tensor
    ...
    #model = misc.construct_class_by_name(**cfg.model, **common_kwargs).train(num_epochs = nepochs, dataloader = dataloader).to(device)
    #model = misc.construct_class_by_name(**cfg.model, **common_kwargs).train(num_epochs = nepochs, dataloader = dataloader).requires_grad_(True).to(device)
    model = misc.construct_class_by_name(**cfg.model, **common_kwargs).requires_grad_(True).to(device)
    ...
    while epoch_idx < nepochs:
        ...
        for img, z in dataloader:
            ...
            if it > 0:      # only evaluate at initialization
                #model = model.train()
                #model = model.train(dataloader = dataloader, num_epochs = nepochs)
                # Model updates
                optimizer.zero_grad()
                #pred = model(z)
                pred = model(img)
